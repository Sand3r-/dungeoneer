...
internal void
AudioPlaySource(Audio *audio, AudioSourceID source_id, Sound *sound,
...
internal void
AudioSetSourcePosition(Audio *audio, AudioSourceID id, v3 position)
{
    AudioSource *source = audio->sources + id;
...
AudioUpdate(Audio *audio)
{
    ...
    for(u32 i = 0; i < AUDIO_SOURCE_MAX; ++i)
    {
        if(audio->sources[i].playing && audio->sources[i].sound &&
           SoundIsLoaded(audio->sources[i].sound))
        {
            ...
            f32 distance_factor = audio->sources[i].distanced
                ? 1 - (V3LengthSquared(V3MinusV3(audio->sources[i].position, audio->listener_position)) / (20 * 20))
                : 1;
                ...
                ++write_sample)
            {
                ...
                f64 target_play_position =
                    start_play_position + (f64)channels * (f64)sample_pitch;
                if(target_play_position >=
                   sound->sample_count)
                {
